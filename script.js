...
window.Webflow.push(function () {
  ...
  function initializeGSAPAnimations() {
    ...
    wrappers.forEach((wrapper) => {
      ...
      const relTextElements = wrapper.querySelectorAll('[data-text-state="rel"]');
      const absTextElements = wrapper.querySelectorAll('[data-text-state="abs"]');
      const relIcons = wrapper.querySelectorAll('[data-icon-state="rel"]');
      const absIcons = wrapper.querySelectorAll('[data-icon-state="abs"]');

      
      relTextElements.forEach(el => gsap.set(el, { x: 0 }));
      relIcons.forEach(el => gsap.set(el, { x: 0, rotation: 0 }));
      absTextElements.forEach(el => gsap.set(el, { x: "-120%" }));
      absIcons.forEach(el => gsap.set(el, { x: "-120%", rotation: 45 }));
      ...
      wrapper.addEventListener("mouseenter", () => {
          ...
          onComplete: () => {
            ...
            if (!wrapper._isHovered) {
              ...
              tlOut.to([
                  ...
                  x: (i, target) => {
                    if ([...relIcons].includes(target)) return 0;
                    if ([...absIcons].includes(target)) return "-120%";
                    if ([...relTextElements].includes(target)) return 0;
                    if ([...absTextElements].includes(target)) return "-120%";
                  },
                  duration: 1,
                  ease: "expo.inOut",
        ...
        tl.to([
          ...
          x: (i, target) => {
            if ([...relIcons].includes(target)) return "120%";
            if ([...absIcons].includes(target)) return "0%";
            if ([...relTextElements].includes(target)) return "120%";
            if ([...absTextElements].includes(target)) return "0%";
          },
          duration: 1,
          ease: "expo.out",
...
window.Webflow.push(function () {
  ...
  const totalCssDuration = 24.5; 
  const pathDuration = totalCssDuration / 4; 
  const animDuration = 4.625; 
  const waitDuration = 1.5; 
...
window.Webflow.push(function () {
  ...
  function applyTabletAnimations() {
    if (window.innerWidth <= 991) {
      const style = document.createElement('style');
      style.id = 'tablet-animations';
      style.textContent = `
      :root {
    ...
    `;
      ...
      const existingStyle = document.getElementById('tablet-animations');
      if (existingStyle) {
  ...
  applyTabletAnimations();

  
  window.addEventListener('resize', applyTabletAnimations);

});
window.Webflow = window.Webflow || [];
window.Webflow.push(function () {
  
  const animatedElementsHotjar = [
  ...
  function resetAnimations() {
    animatedElementsHotjar.forEach(id => {
      const el = document.getElementById(id);
      if (el) {
        el.style.animation = 'none';
      }
